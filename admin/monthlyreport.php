...
<table border="1" cellpadding="0" cellspacing="0" width="1050px">
	...
	<tr>
		<th colspan="100%"><h3>All orders by Customers for the month of  <?php echo $month." / ".$year; ?></h3></th>
	</tr>
	<tr>
	...
	<tr>
		...
		<th>LandName/UPI</th><th>Size</th><th>Amount(FRW)</th><th>Date Uploaded</th><th>Status</th>
	</tr>
	<?php 
	$result = $con ->query("SELECT * FROM customers WHERE orderMonth='$month' AND orderYear='$year'");
	if ($result ->num_rows ==0) {
	 ...
	 <tr>
	 	...
	 	<td colspan="6">
	 		<?php 
	 		$result1 = $con ->query("SELECT * FROM customers WHERE orderMonth='$month' AND orderYear='$year' AND orderChecked='1'");
	 		$result2 = $con ->query("SELECT SUM(Totalamount) AS amount FROM customers WHERE orderMonth='$month' AND orderYear='$year' AND orderChecked='1'");
	 	...
	 	<td colspan="6">
	 		<?php 
	 		$result1 = $con ->query("SELECT * FROM customers WHERE orderMonth='$month' AND orderYear='$year' AND orderChecked='0'");
	 		$result2 = $con ->query("SELECT SUM(Totalamount) AS amount FROM customers WHERE orderMonth='$month' AND orderYear='$year' AND orderChecked='0'");
	 		while ($row = $result2 ->fetch_object()) {
	 		$total = $row ->amount;
	 		}
