...
<table border="1" cellpadding="0" cellspacing="0" width="900px">
	...
	<tr>
		...
		<th>LandName/UPI</th><th>Size</th><th>Amount(FRW)</th><th>Date Uploaded</th><th>Status</th><th>Action</th>
	</tr>
	<?php 
	...
	if ($result ->num_rows ==0) {
		...
		$success = "Dear".$customername.". Payment of ".$Totalamount." for(".$OrderQuantity.")".$OrderUnittype." of ".$orderCommodity." has been received by RAB. Check with ".$customedistrict."".$customesector."RAB Officer in 2 days for your Commodity OR Call 0787751768";
			if ($status ==0) {
				$click = "<a href='?link=3&&status1=0&&success=$success&&tel=$tel&&id=$customerID'class='btn-confirm'><span class='material-symbols-outlined'>check</span>Confirm</a>";
				$status = "Pending";
			}
			if ($status ==1) {
				$click = "<a href='?link=3&&status2=1&&id=$customerID'class='btn-cancel'><span class='material-symbols-outlined'>close</span>Cancel</a>";
				$status = "Cleared";
			}
